            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑作者</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>作者手机号</label>
                                        <input id="zuozhePhone" name="zuozhePhone" class="form-control"
                                               onchange="zuozhePhoneChickValue(this)"  placeholder="作者手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>作者身份证号</label>
                                        <input id="zuozheIdNumber" name="zuozheIdNumber" class="form-control"
                                               onchange="zuozheIdNumberChickValue(this)"  placeholder="作者身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>现住地址</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>出生年月</label>
                                        <input id="zuozheTime-input" name="zuozheTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>作者照片</label>
                                        <img id="zuozhePhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="zuozhePhotoupload"
                                               class="form-control-file">
                                        <input name="zuozhePhoto" id="zuozhePhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#zuozhePhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('zuozhePhotoImg').setAttribute('src',photoUrl);
                document.getElementById('zuozhePhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zuozhe/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzuozhe');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function zuozhePhoneChickValue(e){
        ...
        function zuozheIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                password: "required",
                zuozheName: "required",
                zuozhePhone: "required",
                zuozheIdNumber: "required",
                zuozheAddress: "required",
                zuozheTime: "required",
                zuozhePhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                zuozheName: "作者姓名不能为空",
                zuozhePhone: "作者手机号不能为空",
                zuozheIdNumber: "作者身份证号不能为空",
                zuozheAddress: "现住地址不能为空",
                zuozheTime: "出生年月不能为空",
                zuozhePhoto: "作者照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzuozhe = window.sessionStorage.getItem("addzuozhe");
        if (addzuozhe != null && addzuozhe != "" && addzuozhe != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zuozhe/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
